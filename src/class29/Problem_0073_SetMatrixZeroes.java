...
public class Problem_0073_SetMatrixZeroes {
    ...
    /**
     * 思路： 可以用一个boolean flag[N][M]数组，标记N行和M列是不是要变0
     * 遍历一遍矩阵，填好这个数组，例如来到3行5列位置，假如是0，那么第3行和第5列都要变成0
     * 记录号flag[3][5]=true
     * 然后再遍历一遍falg数组，改变矩阵就行了
     * 
     * 上面的思路比较耗费空间？有没有方法节省空间？
     * 
     * 有的，直接用第0行记录每一列是不是要变0，如果某一列要变0，则将0行某列的位置设置为0；
     * 用第0列记录某一行是不是要变0，如果某一行要变0，则将0列某行的位置设置为0；
     * 
     * 有个需要注意的问题是，上面在在将第0行或者第0列的元素变成0之后，比如直接将0行3列的位置变成了0，
     * 表示的是将第3列变成0，但是出来的时候看见这个位置是0，会将0行也全部变成0。
     * 如果这个位置原本不是0，比如是50，是不能将0行直接变为0的。
     * 列记录的时候也是同样的道理
     * 所以搞两个变量分别记录第0行和第0列是否变为0
     */
     ...
     * 在遍历一遍标记数组，改变原始数组的行和列 --> 空间浪费，为了节省空间，可以用下面的方法。
     * 
    ...
    public static void setZeroes2(int[][] matrix) {
        ...
        for (i = 1; i < matrix.length; i++) {
            for (j = 1; j < matrix[0].length; j++) {
                // 对于格子i,j来说，只要i行或者j列上有一个记录了要标记成0(不管是这一行还是这一列要标记成0，
                // 这个格子都逃不过要被标记成0)，这个格子就得被标记成0
