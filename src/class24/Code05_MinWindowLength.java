...
public class Code05_MinWindowLength {
     ...
     * 
     * all=0之后，L++，滑动窗口吐出一个b，欠账表中b的数量是-3，++之后是-2，是小于等于0的，说明不是一次有效的还款，all不变；
     * all还是0，继续收集一次答案，PK出最小值。 min = R - L + 1;
     * L继续++，滑动窗口又吐出一个b，欠账表中b的数量是-2，++之后是-1，是小于等于0的，说明不是一次有效的还款，all不变
     * all还是0，继续收集一次答案，PK出最小值。 min = R - L + 1;
     * L继续++，滑动窗口又吐出一个c，欠账表中c的数量是0，++之后是1，是大于0的，说明是一次有效的还款，all++
     * all变成1，不收集答案。
     * 
     * R继续++往右，重复上面的过程。。。。
     * 
    ...
    public static int minLength(String s, String t) {
        ...
        while(R != N) {
            ...
            if(all == 0) {
                ...
                // all等于0的时候，L位置的字符，放回存款表后，如果该字符的数量是小于等于0的，说明总的欠账all不变，还是0，需要收集答案
                while(++map[sStr[L]] <= 0) { // ++map[sStr[L]] 表示L位置的数放回存款表
                    L++; // 放回欠账表之后，L要++
                    ans = Math.min(ans, R - L + 1);
                }
                // L位置的字符，放回到了map中，L要++。
                L++;
                // 如果L位置的字符加入到欠账表之后，该字符位置的数大于0了，all要++
                all++;
            }
            R++;
        }
    ...
    public String minWindow(String s, String t) {
        ...
        while(R != N) {
            ...
            if(all == 0) {
                ...
                // 当前L位置的字符放回map后，即使++map[s1[L]] > 0，L要++
                L++;
                all++;
            }
            R++;
